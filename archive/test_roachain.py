...
# add blocks
key_1 = rsa.generate_private_key(public_exponent=65537, key_size=2048)
cert_1 = functions.generate_cert(key_1)
roa_1 = objects.ROA(1, [{'address': '127.0.0.1', 'max_length':24},
                         {'address': '126.0.0.1', 'max_length': 22}], cert_1)

key_2 = rsa.generate_private_key(public_exponent=65537, key_size=2048)
cert_2 = functions.generate_cert(key_2)
roa_2 = objects.ROA(1, [{'address': '127.0.0.1', 'max_length':24},
                         {'address': '126.0.0.1', 'max_length': 22}], cert_2)

signatures = [functions.sign_data(roa_1.to_dict(), key_1).hex(),functions.sign_data(roa_2.to_dict(), key_2).hex()]
test_chain.add_block(Block(1, test_chain.get_latest_block().hash,roa_1.to_dict()))
test_chain.add_block(Block(2, test_chain.get_latest_block().hash,roa_2.to_dict()))
...
for block in test_chain.chain:
        for signature in signatures:
            ...
            print(f"valid?: {functions.verify_roa(block.data,signature)}")
